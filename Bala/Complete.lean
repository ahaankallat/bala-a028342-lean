...
/-!
# Final assembly: Theorem `thm:main`, unconditionally

`Bala/Statements.lean` declares the two combinatorial propositions of the
paper (`prop:product`, `prop:primepower`) as `axiom`s, and `Bala/MainTheorem.lean`
derives Theorem `thm:main` from those axioms. `Bala/ProductCongruence.lean` and
`Bala/PrimePower.lean` separately *prove* those exact propositions (as
`product_congruence'`, `primepower_odd'`, `primepower_two_one'`,
`primepower_two_two'`, `primepower_two_large'`), with no `sorry` and no
`axiom` of their own.

This file closes the loop: it re-plays the *exact same* proof text as
`Bala/MainTheorem.lean` (`reduction`, `reduction_pow`, `combine_prime_powers`,
`main_theorem`), but with every reference to the axioms of `Bala.Statements`
(`product_congruence`, `primepower_odd`, `primepower_two_one`,
`primepower_two_two`, `primepower_two_large`) replaced by the corresponding
*proved theorem* (`product_congruence'`, `primepower_odd'`, etc.) from
`Bala/ProductCongruence.lean` / `Bala/PrimePower.lean`. (We cannot merely
locally alias the unprimed names to the proved theorems, since this file
transitively imports `Bala.Statements` — through `Bala.ProductCongruence`
and `Bala.PrimePower`, which need the axioms as their own target statements
to prove against — so the unprimed names are already taken by the axioms.)
The resulting `main_theorem` is therefore a single, fully self-contained,
unconditional proof of the paper's Theorem `thm:main`, depending on nothing
beyond Lean/Mathlib's standard axioms — see the `#print axioms` check at the
bottom of this file.
-/

/-- Lemma 3.1 (`lem:reduction`). Identical proof to `Bala/MainTheorem.lean`,
using the proved `product_congruence'` in place of the axiom. -/
...
/-- Identical proof to `Bala/MainTheorem.lean`. -/
...
/-- Theorem `thm:main`, the paper's main result, unconditionally proved: this
version depends only on standard Lean/Mathlib axioms (see the `#print axioms`
check below), not on any `axiom` declared for this paper. Identical proof
text to `Bala/MainTheorem.lean`. -/
